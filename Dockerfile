...
RUN apt-get update && \
    ...
    echo "deb https://packagecloud.io/varnishcache/varnish60lts/debian/ bullseye main" | tee /etc/apt/sources.list.d/varnish-cache.list && \
    apt-get update && \
    apt-get install -y libgetdns-dev varnish=6.0.11-1~bullseye varnish-dev=6.0.11-1~bullseye
...
RUN apt-get update && \
    ...
    echo "deb https://packagecloud.io/varnishcache/varnish60lts/debian/ bullseye main" | tee /etc/apt/sources.list.d/varnish-cache.list && \
    apt-get update && \
    apt-get install -y varnish=6.0.11-1~bullseye
